...
class Zhone_Info:
    ...
    def __zone__(self, url:Union[bytes | str]) -> str:
        ...
        Explanation:
        39: Submits a GET Request to the Site with the Domain Name in
        30-54: codes in this line range: Analyzes the status codes in Zone_Requests variable and writes it to the log file
        57-62 Line: Section of Tables Containing Classes with Information Related to IP Addresses
        65:I wrote this line because there are too many spaces in the incoming data:be warned though this removes "all whitespace characters (space, tab, newline, return, form feed)" . In, "this is \t a test\n" will effectively end up as "this is a test"
        66-78:The incoming data string was somehow plain. I extracted the plain data with the re module. I pulled the necessary information.
        81-96:In order to make the incoming data json, I first converted it to dictionaries, then I converted it to json data with "json.dumps", as a result, the data is now Json
        """
        ...
        powered_by = re.search(r"Powered by (.+)", text).group(1)
